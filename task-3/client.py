...
class FTPClient:
    ...
    def get_server_files(self):
        """Fetch list of server files"""
        response = self.send_command({'command': 'list_files'})
        if response.get('status') != 'success':
            print(f"✗ {response.get('message', 'Could not retrieve file list')}")
            return None
        return response.get('files', [])
    
    def choose_server_file(self, files):
        """Ask the user to choose a file from the server list"""
        if not files:
    ...
    def rename_file(self):
        ...
        old_name = self.choose_server_file(files)
        if not old_name:
            return
    ...
    def read_file(self):
        ...
        filename = self.choose_server_file(files)
        if not filename:
            return
        ...
        if response['status'] == 'success':
            ...
            try:
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"✓ {response['message']}")
                print(f"✓ File saved locally as {filepath}")
            except Exception as e:
    ...
    def edit_file(self):
        ...
        new_content = input("Enter new file content: ").strip()
        ...
        if response['status'] == 'success':
            ...
            if history:
                for item in history:
                    ...
                    print(f"  - {action} by {user or 'unknown'} on {file}")
            else:
